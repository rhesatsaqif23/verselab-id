...
export default function ShuffleCard() {
  ...
  const commitProgress = committed
    ? Math.min(1, Math.max(0, (Math.abs(dragX) - COMMIT_THRESHOLD) / (TURN_THRESHOLD - COMMIT_THRESHOLD)))
    : 0
  ...
  const commitNext = useCallback(() => {
    ...
    setOrder((prev) =>
      prev[0] === selectedUnitId
        ? [nextUnit.id, ...prev.filter((id) => id !== nextUnit.id)]
        : prev,
    )
  ...
  const commitPrev = useCallback(() => {
    ...
    setOrder((prev) =>
      prev[0] === selectedUnitId
        ? [prevUnit.id, ...prev.filter((id) => id !== prevUnit.id)]
        : prev,
    )
  ...
  const frontTransition =
    turning || (!dragging && !animating) ? CSS_TRANSITION : 'none'
