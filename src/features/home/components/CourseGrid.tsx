    ...
    <section className="mt-8">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        {courses.map((course) => (
          <button
            key={course.title}
            className="feature-card group flex flex-col items-center gap-3 rounded-2xl border border-border bg-card p-4 text-center transition hover:-translate-y-0.5"
          >
            ...
            <span className="text-xs font-semibold text-foreground">{course.title}</span>
          </button>
        ))}
      </div>
    </section>
